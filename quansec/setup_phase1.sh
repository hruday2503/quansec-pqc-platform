#!/bin/bash
# =============================================================================
# QUANSEC — Phase 1 (IPsec) setup script
...
sudo -u postgres psql -tc "SELECT 1 FROM pg_database WHERE datname='quansec'" | grep -q 1 || \
    sudo -u postgres psql -c "CREATE DATABASE quansec;"

sudo -u postgres psql -c "
DO \$\$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'quansec_user') THEN
        CREATE USER quansec_user WITH PASSWORD 'quansec_pass';
    END IF;
END
\$\$;
GRANT ALL PRIVILEGES ON DATABASE quansec TO quansec_user;
" 
sudo -u postgres psql -d quansec -c "GRANT ALL ON SCHEMA public TO quansec_user;"
...
# psql -U quansec_user -d quansec -f migrations/001_initial.sql
