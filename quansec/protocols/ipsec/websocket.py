"""
websocket.py — real-time WebSocket endpoint for QUANSEC dashboard.
...
'quansec:live'. This endpoint subscribes and forwards to all
...
logger = logging.getLogger("quansec.ipsec.websocket")
router = APIRouter()
...
async def websocket_live(websocket: WebSocket):
    ...
    try:
        ...
        pubsub = redis.pubsub()
        await pubsub.subscribe("quansec:live")

        # Send immediate status on connect
        await websocket.send_json({
            "type": "connected",
            "message": "QUANSEC live feed connected",
            "channel": "quansec:live"
        })
        ...
        try:
            await pubsub.unsubscribe("quansec:live")
            await redis.close()
        except Exception:
            pass
