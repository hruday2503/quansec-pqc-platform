...
events are in /var/log/auth.log. When QUANSEC runs on a different host,
...
logger = logging.getLogger("quansec.ssh.ztaudit")
router = APIRouter(prefix="/api/ssh/zt", tags=["SSH Zero Trust"])

# Where to read cert-auth events from. Override via env.
ZT_AUTH_LOG = os.environ.get("QUANSEC_ZT_LOG", "/var/log/auth.log")
# If set, pull the log from a remote host over the PQC ssh first.
ZT_REMOTE = os.environ.get("QUANSEC_ZT_REMOTE", "")  # e.g. "hd6441@192.168.1.7"
ZT_REMOTE_KEY = os.environ.get("QUANSEC_ZT_KEY", "")
ZT_REMOTE_CERT = os.environ.get("QUANSEC_ZT_CERT", "")
...
# Accepted cert:
#   Accepted publickey for hd6441 from 192.168.1.6 port X ssh2: ED25519-CERT ... ID hruday@quansec (serial 1) CA ED25519 SHA256:...
# Failed cert:
#   Failed publickey for hd6441 from ... ID hruday@quansec (serial 1) CA ED25519 SHA256:...
