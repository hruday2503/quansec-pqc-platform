"""
protocols/ssh/ca.py — QUANSEC Certificate Authority service.

Issues short-lived SSH certificates for users, signed by the QUANSEC CA.
This is the onboarding flow for Zero Trust access:
  ...
  3. QUANSEC signs it into a certificate scoped to an identity + principal(s)
...
logger = logging.getLogger("quansec.ssh.ca")
...
async def ca_info():
    ...
    try:
        ...
        return {"ca_public_key": pub, "fingerprint": fp, "trusted_by": "all QUANSEC PQC SSH servers"}
