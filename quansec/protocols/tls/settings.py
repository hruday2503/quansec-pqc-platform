...
class TlsSettings:
    ...
    def validate(self, *, require_runtime: bool = True) -> List[str]:
        ...
        """
        if not self.enabled:
            return ["TLS module disabled (QUANSEC_TLS_ENABLED=false)"]

        problems: List[str] = []
        warnings: List[str] = []

        if not 1 <= self.port <= 65535:
            problems.append(f"QUANSEC_TLS_PORT must be 1-65535, got {self.port}")
        if self.timeout <= 0:
            problems.append(f"QUANSEC_TLS_TIMEOUT must be positive, got {self.timeout}")
        if self.poll_interval <= 0:
            problems.append(
                f"QUANSEC_TLS_POLL_INTERVAL must be positive, got {self.poll_interval}"
            )
        if not self.server_hostname:
            problems.append("QUANSEC_TLS_SERVER_HOSTNAME must not be empty")
        if not self.groups.strip():
            problems.append("QUANSEC_TLS_GROUPS must not be empty - that would remove enforcement")

        problems += _require_file(self.ca_cert, "QUANSEC_TLS_CA_CERT")
        problems += _require_file(self.server_cert, "QUANSEC_TLS_SERVER_CERT")
        problems += _require_file(self.server_key, "QUANSEC_TLS_SERVER_KEY")
        if self.mtls:
            problems += _require_file(self.client_cert, "QUANSEC_TLS_CLIENT_CERT")
            problems += _require_file(self.client_key, "QUANSEC_TLS_CLIENT_KEY")
        ...
        if extra_groups:
            warnings.append(
                f"QUANSEC_TLS_GROUPS lists fallback groups {extra_groups} alongside "
                f"{self.hybrid_group}. Clients may negotiate classical key exchange; "
                "this is NOT fail-closed."
            )
        if self.early_data:
            warnings.append(
                "QUANSEC_TLS_EARLY_DATA=true enables TLS 1.3 0-RTT, which is replayable."
            )
        if self.log_payloads:
            warnings.append(
                "QUANSEC_TLS_LOG_PAYLOADS=true - local debugging only."
            )
        if not self.mtls:
