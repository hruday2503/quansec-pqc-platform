...
_DEFAULT_CERT_DIR = os.path.expanduser("~/quansec-certs")
...
_DEFAULT_RUNTIME_DIR = os.path.expanduser("~/.quansec/pqc-tls")
...
def _require_secret(name: str) -> str:
    ...
    if not value:
        raise ConfigurationError(
            ...
            f"  Then add it to quansec/.env as:  {name}=<the value>\n\n"
...
class Settings:
    ...
    JWT_ISSUER: str = os.getenv("JWT_ISSUER", "quansec")
    JWT_AUDIENCE: str = os.getenv("JWT_AUDIENCE", "quansec-api")
    ...
    REFRESH_COOKIE_NAME: str = os.getenv("REFRESH_COOKIE_NAME", "quansec_refresh")
    ...
    CSRF_COOKIE_NAME: str = os.getenv("CSRF_COOKIE_NAME", "quansec_csrf")
    ...
    # own process and configured fail-closed on X25519MLKEM768. QUANSEC is the
    ...
    TLS_ENABLED: bool = _flag("QUANSEC_TLS_ENABLED", "true")
    TLS_HOST: str = os.getenv("QUANSEC_TLS_HOST", "127.0.0.1")
    TLS_PORT: int = int(os.getenv("QUANSEC_TLS_PORT", "8443"))
    TLS_SERVER_HOSTNAME: str = os.getenv("QUANSEC_TLS_SERVER_HOSTNAME", "localhost")
    ...
    TLS_RUNTIME_DIR: str = os.getenv("QUANSEC_TLS_RUNTIME_DIR", _DEFAULT_RUNTIME_DIR)
    TLS_NGINX_BIN: str = _runtime_path("QUANSEC_TLS_NGINX_BIN", "nginx/sbin/nginx", TLS_RUNTIME_DIR)
    TLS_OPENSSL_BIN: str = _runtime_path("QUANSEC_TLS_OPENSSL_BIN", "openssl/bin/openssl", TLS_RUNTIME_DIR)
    TLS_NGINX_CONF: str = _runtime_path("QUANSEC_TLS_NGINX_CONF", "conf/nginx.conf", TLS_RUNTIME_DIR)
    TLS_ACCESS_LOG: str = _runtime_path("QUANSEC_TLS_ACCESS_LOG", "logs/access.json.log", TLS_RUNTIME_DIR)
    TLS_ERROR_LOG: str = _runtime_path("QUANSEC_TLS_ERROR_LOG", "logs/error.log", TLS_RUNTIME_DIR)
    TLS_PID_FILE: str = _runtime_path("QUANSEC_TLS_PID_FILE", "run/nginx.pid", TLS_RUNTIME_DIR)
    TLS_EVIDENCE_DIR: str = _runtime_path("QUANSEC_TLS_EVIDENCE_DIR", "evidence", TLS_RUNTIME_DIR)

    # Certificates. Development PKI generated by scripts/generate_tls_certs.py,
    # kept outside the repository with keys mode 600.
    TLS_CERT_DIR: str = os.getenv("QUANSEC_TLS_CERT_DIR", _DEFAULT_CERT_DIR)
    TLS_CA_CERT: str = _cert_path("QUANSEC_TLS_CA_CERT", "ca.crt", TLS_CERT_DIR)
    TLS_SERVER_CERT: str = _cert_path("QUANSEC_TLS_SERVER_CERT", "server.crt", TLS_CERT_DIR)
    TLS_SERVER_KEY: str = _cert_path("QUANSEC_TLS_SERVER_KEY", "server.key", TLS_CERT_DIR)
    TLS_CLIENT_CERT: str = _cert_path("QUANSEC_TLS_CLIENT_CERT", "client.crt", TLS_CERT_DIR)
    TLS_CLIENT_KEY: str = _cert_path("QUANSEC_TLS_CLIENT_KEY", "client.key", TLS_CERT_DIR)
    ...
    TLS_PROTOCOLS: str = os.getenv("QUANSEC_TLS_PROTOCOLS", "TLSv1.3")
    TLS_GROUPS: str = os.getenv("QUANSEC_TLS_GROUPS", "X25519MLKEM768")
    TLS_CIPHERSUITES: str = os.getenv("QUANSEC_TLS_CIPHERSUITES", "TLS_AES_256_GCM_SHA384")
    TLS_HYBRID_GROUP: str = os.getenv("QUANSEC_TLS_HYBRID_GROUP", "X25519MLKEM768")
    ...
    TLS_LOG_PAYLOADS: bool = _flag("QUANSEC_TLS_LOG_PAYLOADS", "false")
