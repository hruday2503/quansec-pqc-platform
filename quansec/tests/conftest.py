"""
tests/conftest.py — shared fixtures for the QUANSEC backend test suite.
...
async def make_user(db_conn):
    ...
    async def _make(role: str = "operator", portal: str = "main",
        ...
        email = f"test-{uuid.uuid4().hex[:12]}@quansec.test"
...
def live_user(database_url, database_available):
    ...
    async def _insert(role: str, portal: str, password: str):
        conn = await asyncpg.connect(database_url)
        try:
            email = f"live-{uuid.uuid4().hex[:12]}@quansec.test"
    ...
    async def _clear():
        try:
            ...
            if redis is not None:
                keys = await redis.keys("quansec:auth:throttle:*")
                if keys:
