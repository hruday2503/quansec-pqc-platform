...
RUNTIME="${QUANSEC_TLS_RUNTIME_DIR:-$HOME/.quansec/pqc-tls}"
NGINX_BIN="${QUANSEC_TLS_NGINX_BIN:-$RUNTIME/nginx/sbin/nginx}"
OPENSSL_BIN="${QUANSEC_TLS_OPENSSL_BIN:-$RUNTIME/openssl/bin/openssl}"
NGINX_CONF="${QUANSEC_TLS_NGINX_CONF:-$RUNTIME/conf/nginx.conf}"
ACCESS_LOG="${QUANSEC_TLS_ACCESS_LOG:-$RUNTIME/logs/access.json.log}"
ERROR_LOG="${QUANSEC_TLS_ERROR_LOG:-$RUNTIME/logs/error.log}"
PID_FILE="${QUANSEC_TLS_PID_FILE:-$RUNTIME/run/nginx.pid}"
EVIDENCE_DIR="${QUANSEC_TLS_EVIDENCE_DIR:-$RUNTIME/evidence}"

TLS_HOST="${QUANSEC_TLS_HOST:-127.0.0.1}"
TLS_PORT="${QUANSEC_TLS_PORT:-8443}"
CERT_DIR="${QUANSEC_TLS_CERT_DIR:-$HOME/quansec-certs}"
CA_CERT="${QUANSEC_TLS_CA_CERT:-$CERT_DIR/ca.crt}"
CLIENT_CERT="${QUANSEC_TLS_CLIENT_CERT:-$CERT_DIR/client.crt}"
CLIENT_KEY="${QUANSEC_TLS_CLIENT_KEY:-$CERT_DIR/client.key}"
SERVER_NAME="${QUANSEC_TLS_SERVER_HOSTNAME:-localhost}"
HYBRID_GROUP="${QUANSEC_TLS_HYBRID_GROUP:-X25519MLKEM768}"
