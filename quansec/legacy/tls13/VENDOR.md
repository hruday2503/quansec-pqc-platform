...
are the authoritative copy used by QUANSEC and are patched for this platform.

> ## Scope change: this is no longer the data plane
>
> QUANSEC's TLS data plane is **NGINX 1.27.5 linked against OpenSSL 3.5.7**
...
| File | Patched |
|---|---|
| `config.py` | yes — extra QUANSEC fields |
| `exceptions.py` | no |
...
| Path | Reason |
|---|---|
| `phase2_server.py`, `phase2_client.py` | `phase2_server.py:47` hardcodes `"tls_native_hybrid": True` as handshake "evidence". QUANSEC must never emit a hybrid claim that was not externally verified, so these compatibility wrappers are excluded outright. |
| `session_tokens.py`, `hybrid_session_keys.py` | HMAC session tokens and HKDF key derivation are not on the TLS transport path. QUANSEC already has JWT + API-key auth in `core/auth.py`. Excluding them also removes the only `cryptography` dependency, leaving this package stdlib-only. |
...
| `examples/` | Demonstration endpoints, including a `POST /certs/regenerate` route. Certificate generation in QUANSEC is CLI-only (`scripts/generate_tls_certs.py`). |
| `check_pqc.py`, `generate_certs.ps1`, `setup.py`, `requirements*.txt` | Platform-specific tooling and packaging not needed in-tree. |

## Patches applied

Each patch is marked in the source with a `# QUANSEC PATCH n:` comment.
...
1. **Structured, redactable logging.** Upstream printed ANSI-coloured, Unicode-decorated
   ...
   module under `quansec.tls.transport`, ASCII-only. Message payloads are logged only
