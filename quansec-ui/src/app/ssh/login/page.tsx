...
export default function SshLoginPage() {
  const [email, setEmail] = useState("ssh-admin@quansec.io");
  ...
  return (
    <div className="min-h-screen flex" style={{ background: "var(--bg-void)" }}>
      <div className="hidden lg:flex w-1/2 relative overflow-hidden items-center justify-center">
        ...
        <div className="absolute -top-32 -left-32 w-[500px] h-[500px] rounded-full blur-3xl pointer-events-none" style={{ background: "var(--lattice-violet-glow)", opacity: 0.7 }} />
        ...
        <div className="relative z-10 max-w-md px-12 text-center">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-8" style={{ background: "var(--lattice-violet-glow)", border: "1px solid var(--lattice-violet-dim)" }}>
            <Terminal size={30} style={{ color: "var(--lattice-violet)" }} strokeWidth={1.8} />
          </div>
          <h1 className="text-3xl font-extrabold mb-4 tracking-tight" style={{ color: "var(--text-primary)" }}>
            SSH Portal
          </h1>
          <p className="text-sm leading-relaxed" style={{ color: "var(--text-secondary)" }}>
            Monitor SSH sessions secured with hybrid X25519 + ML-KEM-768 —
            the OpenSSH post-quantum key exchange at NIST Level 3.
          </p>
          <div className="mt-10 flex items-center justify-center gap-8 font-mono-display">
            <div>
              <div className="text-3xl font-extrabold" style={{ color: "var(--lattice-violet)" }}>768</div>
            ...
            <div>
              <div className="text-3xl font-extrabold" style={{ color: "var(--lattice-violet)" }}>HYBRID</div>
              <div className="text-[10px] font-bold uppercase tracking-widest mt-1" style={{ color: "var(--text-secondary)" }}>+ X25519</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6">
        <form onSubmit={handleSubmit} className="w-full max-w-sm">
          ...
          <h2 className="text-2xl font-extrabold mb-1" style={{ color: "var(--text-primary)" }}>SSH sign in</h2>
          <p className="text-sm mb-8" style={{ color: "var(--text-secondary)" }}>Access your SSH monitoring portal</p>
