...
in your QUANSEC backend yet. Add them with these steps on VM A:

## Step 1 — Copy the file

```bash
cd ~/Downloads/quansec_phase1/quansec
cp /path/to/quansec-ui/backend-additions/api_keys.py protocols/auth_api_keys.py
```
...
```bash
TOKEN=$(curl -s -X POST http://localhost:8000/api/auth/login -d "username=admin@quansec.io&password=Admin@1234" | python3 -c "import sys,json; print(json.load(sys.stdin)['access_token'])")
