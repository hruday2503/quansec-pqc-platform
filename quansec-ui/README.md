...
|---|---|---|
| `NEXT_PUBLIC_QUANSEC_API` | yes | Backend base URL. Falls back to `http://localhost:8000` |
...
`allow_origins` in `quansec/main.py`.
...
`QuansecClient` (`lib/api.ts`) holds the JWT in memory and mirrors it to
...
```ts
quansec.login(email, password, "ssh")
...
| Symptom | Cause | Fix |
|---|---|---|
| Stuck on `AUTHENTICATING…` | `/api/auth/me` failing | Check the backend; clear `localStorage.quansec_token` |
| CORS errors in the console | Origin not in `allow_origins` | Update `quansec/main.py`, or serve on `localhost:3000` |
...
| Calls go to the wrong host | `NEXT_PUBLIC_QUANSEC_API` is build-time | `npm run build` again |
