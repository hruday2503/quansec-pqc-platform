...
import { quanseq, PolicyCompare, Policy } from "@/lib/api";
...
export default function PolicyPage() {
  ...
  const load = async () => {
    const [c, p] = await Promise.all([quanseq.getCompare(), quanseq.getPolicies()]);
    setCompare(c);
    setPolicies(p);
  };
  ...
  const handleApply = async (name: string) => {
    ...
    setToast(null);
    try {
      const res = await quanseq.applyPolicy(name);
