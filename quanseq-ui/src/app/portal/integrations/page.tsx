...
const API_BASE = process.env.NEXT_PUBLIC_QUANSEQ_API || "http://localhost:8000";
...
export default function IntegrationsPage() {
  ...
  const fetchEndpoint = (fmt: string, url: string) => {
    ...
    const sample: Record<string, string> = {
      CEF: `CEF:0|QUANSEQ|IPsec|1.0|policy.apply|Policy applied: pqc-level5|3|src=10.20.0.1 dst=10.20.0.2 cs1Label=kem cs1=ML-KEM-1024`,
      JSON: JSON.stringify({ event: "policy.apply", protocol: "ipsec", policy: "pqc-level5", occurred_at: new Date().toISOString() }, null, 2),
      Syslog: `<134>1 ${new Date().toISOString()} quanseq ipsec - - - policy.apply policy="pqc-level5" pqc_enabled=true`,
    };
    const blob = new Blob([sample[fmt] ?? ""], { type: "text/plain" });
    const blobUrl = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = blobUrl; a.download = `quanseq-siem-events.${fmt.toLowerCase()}`; a.click();
