...
curl https://api.quanseq.io/api/ipsec/stats \\
  -H "Authorization: Bearer $QUANSEQ_API_KEY"

# Apply post-quantum policy to your tunnel
curl -X POST https://api.quanseq.io/api/ipsec/policies/apply \\
  -H "Authorization: Bearer $QUANSEQ_API_KEY" \\
  ...
  node: {
    label: "Node.js",
    code: `const QUANSEQ_API_KEY = process.env.QUANSEQ_API_KEY;
const BASE_URL = "https://api.quanseq.io";

async function getTunnelStatus() {
  const res = await fetch(\`\${BASE_URL}/api/ipsec/stats\`, {
    headers: { Authorization: \`Bearer \${QUANSEQ_API_KEY}\` },
  });
  return res.json();
}
...
QUANSEQ_API_KEY = os.environ["QUANSEQ_API_KEY"]
BASE_URL = "https://api.quanseq.io"

def get_tunnel_status():
    response = requests.get(
        f"{BASE_URL}/api/ipsec/stats",
        headers={"Authorization": f"Bearer {QUANSEQ_API_KEY}"},
    )
...
print(f"PQC coverage: {status['pqc_coverage']}%")`,
  ...
  "wss://api.quanseq.io/api/ws/live"
);
