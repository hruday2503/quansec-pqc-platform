...
export default function IntegrationsPage() {
  ...
  const fetchEndpoint = (fmt: string, url: string) => {
    ...
    const sample: Record<string, string> = {
      CEF: `CEF:0|QUANSEQ|SSH|1.0|policy.apply|Policy applied: hybrid-pqc|3|src=10.20.0.1 cs1Label=kex cs1=mlkem768x25519-sha256`,
      JSON: JSON.stringify({ event: "policy.apply", protocol: "ssh", policy: "hybrid-pqc", occurred_at: new Date().toISOString() }, null, 2),
      Syslog: `<134>1 ${new Date().toISOString()} quanseq ssh - - - policy.apply policy="hybrid-pqc" pqc_enabled=true`,
    };
