// lib/api.ts — QUANSEQ API client

import { SHOWCASE_MODE } from "@/lib/mock/mode";
import { MockQuanseqClient } from "@/lib/mock/client";
...
class QuanseqClient {
  ...
  private readonly LEGACY_TOKEN_KEY = "quanseq_token";
  ...
  private readCsrfCookie(): string | null {
    ...
    const match = document.cookie.match(/(?:^|;\s*)quanseq_csrf=([^;]+)/);
...
export const quanseq: QuanseqClient = SHOWCASE_MODE
  ? (new MockQuanseqClient() as unknown as QuanseqClient)
  : new QuanseqClient();
