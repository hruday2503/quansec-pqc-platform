 ...
 * QUANSEQ client and the raw-fetch endpoints used by the SSH pages) derives
...
const ztEvents = Array.from({ length: 6 }).map((_, i) => ({
  ...
  ca_fingerprint: "SHA256:9fJ2k...QUANSEQ-CA",
...
const tlsSessions: TlsSession[] = Array.from({ length: 14 }).map((_, i) => {
  ...
  return {
    ...
    client_s_dn: i % 6 === 0 ? "CN=client-svc,O=QUANSEQ" : null,
    server_name: "quanseq.local",
...
function computeTlsStatus(): TlsStatus {
  ...
  return {
    ...
    service: { host: "0.0.0.0", port: 8444, running: true, pid: 4821, listening: true, config_path: "/etc/quanseq/nginx.conf", config_sha256: "9c2f...ab41", error: null },
    build: { runtime_dir: "/opt/quanseq/tls", nginx_binary: "/usr/sbin/nginx", nginx_version: "1.27.3", nginx_openssl: "OpenSSL 3.5.0", openssl_binary: "/usr/bin/openssl", openssl_version: "OpenSSL 3.5.0", system_openssl: "OpenSSL 3.5.0", hybrid_group_available: true, runtime_built: true },
    evidence: {
...
const tlsCertificate: TlsCertificate = {
  server_name: "quanseq.local",
  subject: "CN=quanseq.local, O=QUANSEQ",
  issuer: "CN=QUANSEQ Development CA, O=QUANSEQ",
  serial_number: "7A:3F:2C:19:BE:04",
  signature_algorithm: "ecdsa-with-SHA384",
  public_key_algorithm: "EC",
  key_size: 384,
  san: ["quanseq.local", "localhost"],
...
function runTlsProbeSuite(): TlsProbeSuite {
  const results: TlsProbeResult[] = [
    { probe_type: "hybrid-positive", expected_outcome: "connect", actual_outcome: "connect", passed: true, description: "Hybrid-capable client connects", target_host: "quanseq.local", target_port: 8444, negotiated_group: "X25519MLKEM768", negotiated_cipher: "TLS_AES_256_GCM_SHA384", tls_protocol: "TLSv1.3", verify_result: "OK", http_status: 200, openssl_binary: "/usr/bin/openssl", openssl_version: "OpenSSL 3.5.0", command: "openssl s_client -groups X25519MLKEM768", exit_code: 0, stdout_excerpt: "Server Temp Key: X25519MLKEM768", evidence_path: null, duration_ms: 84, run_at: nowIso() },
    { probe_type: "tls13-required", expected_outcome: "connect", actual_outcome: "connect", passed: true, description: "TLS 1.3 client connects", target_host: "quanseq.local", target_port: 8444, negotiated_group: "X25519MLKEM768", negotiated_cipher: "TLS_AES_256_GCM_SHA384", tls_protocol: "TLSv1.3", verify_result: "OK", http_status: 200, openssl_binary: "/usr/bin/openssl", openssl_version: "OpenSSL 3.5.0", command: "openssl s_client -tls1_3", exit_code: 0, stdout_excerpt: null, evidence_path: null, duration_ms: 71, run_at: nowIso() },
  ];
...
export const showcaseStore = {
  ...
  getScore: (protocol: "ipsec" | "ssh" | "tls") => {
    ...
    // tls — downgrade_resistant is unconditionally false: QUANSEQ cannot
