...
const SESSION_KEY = "quanseq_showcase_session";
...
/**
 * Serves every QuanseqClient method from the in-memory showcase store instead
 * of the network. Method signatures mirror QuanseqClient exactly so this can
 * be swapped in for it at the export site in lib/api.ts.
 */
export class MockQuanseqClient {
  ...
  async me() {
    await showcaseLatency(60, 180);
    const session = readSession() ?? { id: 1, email: "admin@quanseq.io", role: "admin", portal: null };
    return {
      id: session.id,
      email: session.email,
      role: session.role,
