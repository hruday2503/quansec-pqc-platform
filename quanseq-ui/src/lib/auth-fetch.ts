 ...
 * from `localStorage`. The token now lives in memory on the `quanseq` client
...
import { quanseq } from "./api";

export const API_BASE =
  process.env.NEXT_PUBLIC_QUANSEQ_API || "http://localhost:8000";
...
export async function authedFetch(
  ...
  const refreshed = await quanseq.refresh();
