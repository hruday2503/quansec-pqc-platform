...
#
# QUANSEQ PATCH 1: stdlib logging instead of coloured print(); payload redaction.
# QUANSEQ PATCH 2: honest negotiated-group reporting.
# QUANSEQ PATCH 3: _require_hybrid_tls_runtime renamed and widened.
...
def get_session_info(sock: ssl.SSLSocket,
    ...
    QUANSEQ PATCH 2 — on negotiated_group:
...
def redact_payload(message: str, config: TLSConfig) -> str:
    """
    QUANSEQ PATCH 1/6 — return a loggable stand-in for an application payload.
...
def _require_hybrid_capable_runtime(config: TLSConfig) -> None:
    ...
    if current < MIN_HYBRID_OPENSSL:
        raise TLSVersionError(
            ...
            "Run with QUANSEQ_TLS_REQUIRE_HYBRID=false, or use a Python linked "
    ...
    if not runtime_supports_group(config.hybrid_group, config.openssl_bin):
        raise TLSVersionError(
            ...
            "Run with QUANSEQ_TLS_REQUIRE_HYBRID=false, or point "
            "QUANSEQ_TLS_OPENSSL_BIN at an OpenSSL that provides the group."
        )
