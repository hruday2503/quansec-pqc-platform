#!/usr/bin/env bash
# scripts/start-pqc-tls.sh — start the QUANSEQ post-quantum TLS data plane.
#
...
# than a shell heredoc means the running config and the config QUANSEQ reports
...
echo "    Policy   : TLSv1.3 only, Groups=${QUANSEQ_TLS_GROUPS:-$HYBRID_GROUP}, mTLS=$MTLS_FLAG"

cd "$BACKEND_DIR"
QUANSEQ_TLS_MTLS="$MTLS_FLAG" "$PYTHON" - <<'PY' || die "start failed"
import logging, sys
sys.path.insert(0, ".")
