...
#
# Builds the QUANSEQ post-quantum TLS data plane:
#
...
#
# ~/.quanseq/pqc-tls is still an isolated, project-scoped runtime: it is used by
# nothing else, it is outside every system prefix, and deleting it undoes the
# build completely. Override with QUANSEQ_TLS_RUNTIME_DIR if your checkout path
# has no spaces and you would rather keep it in-tree.
DEFAULT_RUNTIME="$HOME/.quanseq/pqc-tls"
RUNTIME="${QUANSEQ_TLS_RUNTIME_DIR:-$DEFAULT_RUNTIME}"
...
Choose a whitespace-free path:
    ...
    QUANSEQ_TLS_RUNTIME_DIR=$HOME/.quanseq/pqc-tls bash scripts/build-pqc-tls-runtime.sh
EOF
        exit 1
        ;;
esac
