...
logger = logging.getLogger("quanseq.metrics")
...
async def prometheus_metrics(conn: asyncpg.Connection = Depends(get_db)):
    ...
    try:
        ...
        metric("quanseq_ipsec_tunnels_total", ip_total, "Total IPsec tunnels")
        metric("quanseq_ipsec_tunnels_established", ip_est, "Established IPsec tunnels")
        metric("quanseq_ipsec_tunnels_pqc", ip_pqc, "PQC-enabled IPsec tunnels")
        metric("quanseq_ipsec_pqc_coverage_percent", round((ip_pqc / ip_est * 100) if ip_est else 0, 1), "IPsec PQC coverage %")
        metric("quanseq_ipsec_bytes_in_total", ip_bytes_in, "IPsec bytes received", "counter")
        metric("quanseq_ipsec_bytes_out_total", ip_bytes_out, "IPsec bytes sent", "counter")
    ...
    try:
        ...
        metric("quanseq_ssh_sessions_total", sh_total, "Total SSH sessions seen")
        metric("quanseq_ssh_sessions_active", sh_active, "Active SSH sessions")
        metric("quanseq_ssh_sessions_pqc", sh_pqc, "PQC-enabled active SSH sessions")
        metric("quanseq_ssh_pqc_coverage_percent", round((sh_pqc / sh_active * 100) if sh_active else 0, 1), "SSH PQC coverage %")
        metric("quanseq_ssh_bytes_sent_total", sh_bs, "SSH bytes sent", "counter")
        metric("quanseq_ssh_bytes_received_total", sh_br, "SSH bytes received", "counter")
    except Exception as e:
        logger.debug(f"ssh metrics: {e}")

    # TLS metrics
    #
    # quanseq_tls_handshakes_pqc counts only observations backed by verified
    ...
    try:
        ...
        metric("quanseq_tls_handshakes_total", tls_total, "TLS handshake observations", "counter")
        metric("quanseq_tls_handshakes_successful", tls_ok, "Successful TLS handshakes", "counter")
        metric("quanseq_tls_handshakes_failed", tls_total - tls_ok, "Failed TLS handshakes", "counter")
        metric("quanseq_tls_handshakes_pqc", tls_pqc,
               "TLS handshakes with verified hybrid PQC evidence", "counter")
        metric("quanseq_tls_pqc_coverage_percent",
               round((tls_pqc / tls_ok * 100) if tls_ok else 0, 1), "TLS PQC coverage %")
        metric("quanseq_tls13_handshakes", tls_13, "Handshakes negotiating TLS 1.3", "counter")
        if latencies:
            metric("quanseq_tls_handshake_duration_ms",
        ...
        metric("quanseq_tls_hybrid_verifications_total", evidence or 0,
        ...
        metric("quanseq_tls_hybrid_enforced", 0,
    ...
    try:
        ...
        if exists:
            ...
            metric("quanseq_zt_auth_accepted_total", zt_acc, "Zero Trust cert auths accepted", "counter")
            metric("quanseq_zt_auth_rejected_total", zt_rej, "Zero Trust cert auths rejected", "counter")
    except Exception as e:
