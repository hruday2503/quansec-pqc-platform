...
logger = logging.getLogger("quanseq.tls.router")
...
def _require_enabled():
    ...
    if not settings.enabled:
        raise HTTPException(
            status_code=503,
            detail="TLS module is disabled (QUANSEQ_TLS_ENABLED=false)",
        )
    return settings
...
async def tls_dataplane_echo(
    request: Request,
    tls_protocol: Optional[str] = Header(None, alias="X-QUANSEQ-TLS-Protocol"),
    cipher: Optional[str] = Header(None, alias="X-QUANSEQ-TLS-Cipher"),
    negotiated_group: Optional[str] = Header(None, alias="X-QUANSEQ-TLS-Group"),
    client_groups: Optional[str] = Header(None, alias="X-QUANSEQ-TLS-Curves"),
    client_verify: Optional[str] = Header(None, alias="X-QUANSEQ-TLS-Verify"),
    server_name: Optional[str] = Header(None, alias="X-QUANSEQ-TLS-ServerName"),
):
    """
    ...
    via_nginx = any(h.lower().startswith("x-quanseq-tls-")
