...
QUANSEQ's claim about TLS rests on one question: **is the policy on disk the
...
logger = logging.getLogger("quanseq.tls.policy")

POLICY_NAME = "quanseq-pqc-tls"
...
async def apply_policy(conn: asyncpg.Connection, *, applied_by: str,
    ...
    if not view.fail_closed:
        ...
        return {
            ...
            "detail": (
                f"QUANSEQ_TLS_GROUPS={view.groups!r} with "
                f"QUANSEQ_TLS_PROTOCOLS={view.protocols!r} would allow a classical "
