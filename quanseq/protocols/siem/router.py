...
Exports QUANSEQ security events (policy changes, attacks, Zero Trust
...
logger = logging.getLogger("quanseq.siem")
router = APIRouter(prefix="/api/siem", tags=["SIEM Export"])

CEF_HEADER = "CEF:0|QUANSEQ|PQC-Platform|1.0"
...
def _to_syslog(e) -> str:
    ...
    return f"<{pri}>1 {ts} quanseq pqc-platform - - - {e['action']} {e.get('detail','')}"
