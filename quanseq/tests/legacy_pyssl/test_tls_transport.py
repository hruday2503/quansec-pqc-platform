...
a failure here means the transport genuinely does not do what QUANSEQ reports it
does.
...
def test_encrypted_message_exchange_round_trips(running_server):
    ...
    with TLSClient(config) as client:
        assert client.send("hello quanseq") == "Echo: hello quanseq"
...
def test_certificate_chain_verifies_against_the_ca(running_server, pki):
    ...
    assert cert["issuer_cn"] == "QUANSEQ TLS Development Root CA"
