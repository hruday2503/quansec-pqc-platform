"""
tests/conftest.py — shared fixtures for the QUANSEQ test suite.

Every TLS test generates its own throwaway PKI into a temp directory and binds
an ephemeral port. No fixture reads or writes QUANSEQ_TLS_CERT_DIR, so running
...
def pki(tmp_path_factory) -> CertManager:
    ...
    """
    cert_dir = tmp_path_factory.mktemp("quanseq-test-pki")
...
def api_client(database_url, database_available):
    ...
    with unauthenticated_client(database_url) as client:
        admin = {"id": 1, "email": "admin@quanseq.io", "role": "admin"}
