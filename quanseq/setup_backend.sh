#!/usr/bin/env bash
# =============================================================================
# QUANSEQ Backend Setup — PostgreSQL + Redis + Python Env + Migrations + Seed
# =============================================================================
# Usage:
...
#   2. Creates the quanseq_user and quanseq_db in PostgreSQL
...
DB_USER="${DB_USER:-quanseq_user}"
DB_PASS="${DB_PASS:-quanseq_secret}"
DB_NAME="${DB_NAME:-quanseq_db}"
DB_HOST="${DB_HOST:-localhost}"
DB_PORT="${DB_PORT:-5432}"
ADMIN_PASSWORD="${ADMIN_PASSWORD:-Admin@QuanSeq2024!}"
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    cat > "$ENV_FILE" <<ENVEOF
# QUANSEQ Backend Environment — auto-generated by setup_backend.sh
...
redis-cli ping
redis-cli set quanseq:setup:verified "$(date -Iseconds)" EX 86400 >/dev/null
success "Redis key set successfully"

# =============================================================================
echo ""
echo -e "${BOLD}${GREEN}══════════════════════════════════════════════════════════${RESET}"
echo -e "${BOLD}${GREEN}  ✅  QUANSEQ Backend Setup Complete!${RESET}"
echo -e "${BOLD}${GREEN}══════════════════════════════════════════════════════════${RESET}"
echo ""
